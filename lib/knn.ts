...
export async function getKNearestNeighborsByUserId(userId: string, k: number) { 
    ...
    const usersWithSimilarityScores = otherUsers.map((user) => {
        ...
        const combinedSimilarityScore = combinedSimilarity(vector1, vector2, targetUser.jobTitle, user.jobTitle);
    ...
    // sort the combined similarity in descending order
    usersWithSimilarityScores.sort((a, b) => b.similarity - a.similarity);

    // get the top k users
    const topKUsersWithSimilaritScore = usersWithSimilarityScores.slice(0, k);
    return topKUsersWithSimilaritScore;
}